...
export default function DebugCamera() {
  ...
  async function takePic(e) {
    if (e.type === "capture") {
      ...
      const data = await RNFS.readFile(uri, 'base64').then(res => { return res });
      setPhoto(data);
    }
  }
  ...
  if (permissions === "denied") {
    return (
      <View style={{ height: "100%", alignItems: "center", justifyContent: "center", backgroundColor: "black"}}>
        <Text style={{color: "white"}}>Verifique se concedeu as permissões necessárias.</Text>
      </View>
    );
  }
  ...
  if (photo) {
    return (
      <View>
        <Image style={{width: "100%", height: "92.9%"}} source={{ uri: "data:image/jpeg;base64," + photo }} />
        <Button
